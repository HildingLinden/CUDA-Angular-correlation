...
int main(void) {
	...
	float *h_ascD = (float *)calloc(nCoordinatePairsD+8, sizeof(float));
	float *h_decD = (float *)calloc(nCoordinatePairsD+8, sizeof(float));
	...
	if (AVX) {
		...
		{
			/* Vectorized part */
			float *resultArr = (float *)calloc(nCoordinatePairsD+8, sizeof(float));
			...
			#pragma omp for schedule(dynamic, 1) nowait
				for (int i = 0; i < 100000; i++) {
					...
					for (int j = i+1; j < 100000; j +=8) {
						__m256 asc2 = _mm256_loadu_ps(h_ascD+j);
						__m256 dec2 = _mm256_loadu_ps(h_decD+j);

						// sin(dec1) and cos(dec1)
						__m256 sinDec1 = _mm256_sin_ps(dec1);
						__m256 sinDec2 = _mm256_sin_ps(dec2);

						// sin(dec2) and cos(dec2)
						__m256 cosDec1 = _mm256_cos_ps(dec1);
						__m256 cosDec2 = _mm256_cos_ps(dec2);
